...
- Empty `enum ExponentialBackoff: Sendable` used as namespace → use `struct`/`final class` with private init.
...
- Error descriptions hard-coded English, no localisation.

## Tokenizer.swift
- L18: `enum EstonianTokenizer` with only static methods – namespace misuse.
